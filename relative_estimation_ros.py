...
class VisionEstimation(object):
    def __init__(self):
        ...
        self._mask_led_converted = Image()
        ...
        rospy.Subscriber("/image_raw", Image, self._set_buff_img)
    ...
    def _mask(self):
        ...
        img_undistort = cv2.remap(img_raw, map_func[0], map_func[1], cv2.INTER_AREA)
         ...
         # マスク処理パラメータ
        led_min = np.array([0, 180, 0], np.uint8)
        ...
        mask_led = cv2.inRange(img_raw, led_min, led_max)
        ...
        # make copy of image
        led_and_contours = np.copy(img_raw)
        ...
        self._mask_led_converted = self._bridge.cv2_to_imgmsg(mask_led, encoding="8UC1")
        self._pub_mask_img.publish(self._mask_led_converted)
        self._error_count = 0
